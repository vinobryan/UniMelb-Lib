...
/**
 * return true if life lost in this node
*/
...
/**
 * heuristic calculation of a node
*/
...
/**
 * get current rewards based on discount, scores
 * and heuristic
*/
float get_reward ( node_t* n ){
	float reward = 0;
...
/**
 * Apply an action to node n and return a new node resulting from 
...
/**
 * Select largest score of action randomly if there are multiple
*/
...
move_t get_next_move( state_t init_state, int budget, 
                     ...
                     int *expanded_nodes, int *generated_nodes, 
                     int *max_depth){
    ...
    while (h.count != 0){
        ...
        heap_push(&visited, pop_node);
        
        *expanded_nodes = *expanded_nodes + 1;
        if (visited.count < budget){
            for (unsigned move = 0; move < 4; move++){
                ...
                *generated_nodes = *generated_nodes + 1;
                ...
                if (success){
                    node_t *propagate_node = 
                        propagate_back_score_to_first_action(new_node);
                    move_t propagate_move = propagate_node->move;
                    if (propagation == max){
                        if (new_node->state.Points > 
                            best_action_score[propagate_move]){
                            best_action_score[propagate_move] = 
                                new_node->state.Points;
                        }
                    }else if (propagation == avg){
                        best_action_score[propagate_move] = 
                            best_action_score[propagate_move] * 
                            propagate_node->num_childs;
                        propagate_node->num_childs++;
                        best_action_score[propagate_move] += 
                            new_node->state.Points;
                        best_action_score[propagate_move] = 
    ...
    while (visited.count != 0){
        node_t *pop_node = heap_delete(&visited);
        free(pop_node);
    }
	...
	sprintf(stats, "%sScore Left %f Right %f Up %f Down %f", 
            ...
            best_action_score[right], 
            best_action_score[up], 
            best_action_score[down]);
    
	return best_action;
}

/**
 * Find the initial node from current node 
*/
