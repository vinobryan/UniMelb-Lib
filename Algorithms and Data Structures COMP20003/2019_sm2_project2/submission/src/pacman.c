...
void write_data(){
    int expanded_per_second;
    double max_tile_score;
    char temp[MAX_DATA_LENGTH];
    FILE *fp = fopen(OUTPUT_FILE_NAME, "a+");
    assert(fp!=NULL);
    expanded_per_second = expanded_nodes / time_spent;
    
    fputs("Propagation = ", fp);
    if(propagation == max)
        sprintf(temp, "max");
    if(propagation == avg)
        sprintf(temp, "avg");
    ...
    fputs("Level = ", fp);
    sprintf(temp, "%d", LevelNumber);
    fputs(temp, fp);
    fputs("\n", fp);
    
    fputs("Budget = ", fp);
    sprintf(temp, "%d", budget);
    fputs(temp, fp);
    fputs("\n", fp);
    ...
    fputs("TotalGenerated = ", fp);
    sprintf(temp, "%d", generated_nodes);
    fputs(temp, fp);
    fputs("\n", fp);

    fputs("TotalExpanded = ", fp);
    sprintf(temp, "%d", expanded_nodes);
    fputs(temp, fp);
    fputs("\n", fp);

    fputs("Time = ", fp);
    sprintf(temp, "%.02f", time_spent);
    fputs(temp, fp);
    fputs(" seconds", fp);
    fputs("\n", fp);
...
void ExitProgram(const char *message) {
    ...
    write_data(propagation, budget, max_depth, expanded_nodes, generated_nodes, time_spent, Points);
...
void MainLoop() {
    ...
    do {
	    ...
	    if(ai_run){
             ...
             */
            move_t selected_move = get_next_move( current_state, budget, propagation, ai_stats, &expanded_nodes, &generated_nodes, &max_depth);
