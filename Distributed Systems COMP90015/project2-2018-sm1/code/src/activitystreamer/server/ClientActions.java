...
public class ClientActions {
	...
	public Response login(Connection con) {
		...
		if (Control.getInstance().getUsers().usernameExist(sUser.getUsername())){
			if (Control.getInstance().getUsers().userMatch(sUser.getUsername(), sUser.getSecret()) || sUser.isAnonymous()) {
				con.setClient();
				con.setUser(sUser);
				return loginSuccess();
			}else {
	...
	public Response register(Connection con) {
		...
		if (Control.getInstance().getUsers().usernameExist((String) content.get(Settings.USERNAME))) {
			return failMsg;
		} else {
