...
double max_signal_all(store_t **data_store, double x,
                       double y, int max_number);
...
int main(int argc, char *argv[]){
    ...
    /* Allocate memory */
    if ((data_store = (store_t**)malloc((MAX_WAPS + MAX_POINTS)
                                         * sizeof(store_t*))) == NULL) {
            malloc_fail();
    }
    while (TRUE){
        char id = getchar();
        if (id == STAGE_1_ID && stage_1_end == FALSE){
            scanf("%lf %lf %lf %lf\n", &x_cor, &y_cor,
                  &trans_power, &signal_frequency);
