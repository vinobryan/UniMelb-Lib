...
public class Control extends Thread {
	...
	private static ArrayList<ServerDetails> parentServers;
	...
	public Control() {
		...
		parentServers = new ArrayList<ServerDetails>();
		// start a listener
		try {
	...
	public boolean initiateConnection(){
		...
		if(Settings.getRemoteHostname()!=null){
			try {
				...
				connections.add(c);
				c.setParent();
				parentServers.add(new ServerDetails(Settings.getRemoteHostname(), (long) Settings.getRemotePort(), c));
				frame.setAction(Settings.getRemoteHostname());
...
//				System.exit(-1);
	...
	public synchronized void connectionClosed(Connection con){
		...
		if (con.isParent()) {
			...
			for (int i=0; i<=parentServers.size(); i++) {
				if (tryOtherParent(parentServers.get(i).getHost(), parentServers.get(i).getPort())){
					success = true;
					break;
				}else {
					parentServers.remove(i);
				}
			}
			if (!success) {
				while (!this.initiateConnection());
			}
		}
		if(!term) connections.remove(con);
	}
	
	private boolean tryOtherParent(String host, Long port) {
	...
	public boolean doActivity(){
		if (frame.doActivitySelected()) {
			Broadcaster.getInstance().broadcastAnnounce(Settings.serverId, getClients().size(), Settings.getLocalHostname(), Settings.getLocalPort());
	...
	public ArrayList<ServerDetails> getParentServers() {
		return parentServers;
	}
	
	public void addParentServers(ServerDetails c) {
		parentServers.add(0, c);
	}
	...
	public void setSd(ServerDetails serverDetail) {
		sd.add(serverDetail);
	}
}
